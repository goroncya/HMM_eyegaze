...
report_table_main <- dist_summary %>% filter(MediaID %in% selected_media) %>% arrange(desc(mean_dist_total)) %>%
  mutate(media_role=case_when(
      ...
      TRUE ~ "selected")) %>%
  dplyr::select(MediaID, media_role, mean_dist_occ, mean_dist_centers, mean_dist_trans, mean_dist_total, median_dist_total, sd_dist_total, n)
...
report_table_centers <- center_shift_summary %>% filter(MediaID %in% selected_media) %>% arrange(MediaID, desc(mean_shift)) %>% group_by(MediaID) %>%
  slice_head(n=5) %>% ungroup() %>% dplyr::select(MediaID, clean_state, n, mean_dx, mean_dy, mean_shift)
...
report_table_occ <- occ_shift_summary %>% filter(MediaID %in% selected_media, n>=10) %>% mutate(abs_delta = abs(mean_delta_occ)) %>% arrange(MediaID, desc(abs_delta)) %>%
  group_by(MediaID) %>% slice_head(n=6) %>% ungroup() %>% dplyr::select(MediaID, ref_state, type, n, mean_occ_clean, mean_occ_media, mean_delta_occ)
...
report_table_trans <- transition_shift_summary %>% filter(MediaID %in% selected_media) %>%
  ...
  slice_head(n=8) %>% ungroup() %>% dplyr::select(MediaID, clean_from, clean_to, mean_trans_clean, mean_trans_media, mean_delta_trans)
